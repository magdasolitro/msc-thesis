...
cache = array[value];		// look-up
	...
	\begin{itemize}
		...
		\item \textbf{oo7} \cite{Wang2019} is an analysis tool that detects Spectre gadgets through \textit{taint analysis}
	...
	Before diving into the description and comparison between these tools, 
	...
	\section{FastSpec}
	\textit{FastSpec} \cite{Tol2021} is a recent tool based on deep learning that is able to identify Spectre gadgets much faster than common rule-based methods (such as...). The need to employ deep learning techniques comes from an important limitation encountered in other analysis tools: the detection process aims at recognising vulnerable code sections by matching them with the Spectre gadgets that have been identified so far, which amount to 15. However, this strategy falls short at recognising subtle code variations occuring in different programs.
	The paper by Tol et al. targets this problem through the following contributions:
	\begin{itemize}
		\item They extend the current set of 15 gadgets to 1 million gadgets by applying a mutational fuzzing technique, creating an appropriate dataset for the deep learning algorithm.
		...
		\item They introduce \textit{FastSpec}, based on supervised neural network embeddings to identify potential gadgets faster that rule-based methods.
	\end{itemize}
