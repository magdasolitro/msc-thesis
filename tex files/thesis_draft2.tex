...
uint8_t foo(uint32_t x) { 
	...
	This section is the core of the thesis: here you will find a precise definition of the research question that I decided to tackle, a description of the methodology that I followed, and the discussion concerning the results I obtained.
	
	As I previously mentioned at the beginning of this chapter, the goal of my work is to test the OpenSSL library that implements AES with different tools and verify the presence of vulnerabilities. More precisely, I try to answer the following questions:
	\begin{enumerate}
		...
		\item Comparing the results obtained on AES in its OpenSSL v1.0.0\footnote{\url{https://github.com/openssl/openssl/tree/91bad2b09eb2ad77da8aca29f80f2d0677e75423}} implementation (last revised in 2015, before the discovering of Spectre) in its OpenSSL v3.0.0 implementation (the latest version), are there any differences? 
	\end{enumerate}
	...
	Firstly, we tested the libraries with \textsc{\textsc{\textsc{KLEESpectre}}}: all I had to do was to write a brief piece of C code, where we call the functions needed to set up the AES encryption, and then we call a specific function from KLEE to declare what inputs are not known prior execution.
	
	For example, the program shown below was used to test AES in CBC mode of operation (see Appendix \ref{appendix:aes}). The most notable variables are:
	\begin{itemize}
		\item \texttt{in}, the message to encrypt. Since we are working with AES-128, the size of the string can be at most 16 characters (C uses the ASCII encoding for strings, where each character is consumes 8 bits).
...
int main(){
	...
	In this program, we use a specific \textsc{KLEE} function called \texttt{klee\_make\_symbolic} to declare what input needs to be abstracted. In this case, we must choose \texttt{ivec}: this value is random and must be refreshed at every encryption, so it cannot be determined by simply scanning the source code. Plus, the adversary can carry out a Spectre attack by manipulating its value: for example, they can submit the same value multiple times, so when the CPU encounters a branch conditioned on the value of the initialisation vector, it will guess the execution a certain branch. The function takes the following parameters:
	\begin{itemize}
		\item \texttt{\&ivec} is the address of the abstract variable;
		\item \texttt{sizeof(ivec)} represents, clearly, its size;
		\item \texttt{"ivec"} is the name with which \textsc{KLEESpectre} will represent the abstract input when printing the output.
	\end{itemize}
	...
	\paragraph{Testing OpenSSL v1} This paragraph shows the results obtained for AES in three different modes of operation, namely Cipher Block Chaining (CBC), Output FeedBack (OFB), and Counter (CTR) mode, testing OpenSSL v1 (more precisely, version 1.0.0). First, I present all the results produced by \textsc{KLEESpectre} and describe my first impressions on them. Then, I will go deeper and try to make sense out of unexpected results.
	
	The first victim of my analysis is AES-CBC: the results are depicted in Figure \ref{fig:result_cbc_v1} and show 4 Spectre vulnerabilities. 
	...
	\end{itemize}
	The next step is going through each and every vulnerability detected by \texttt{KLEESpectre} to check if the results are reasonable and the snippets regarded as "vulnerable" can potentially be exploited in a Spectre attack. The leaking branch is in \texttt{aes\_core.c}, a file that implements a set of functions used in all modes of operations of AES.
	\begin{minipage}{.75\textwidth}
		\begin{lstlisting}[caption=\texttt{aes\_core.c},frame=tlrb, firstnumber=653]{Name}
if (bits == 128) {
	while (1) {
		temp  = rk[3];
		rk[4] = rk[0] ^
...
rk[5] = GETU32(userKey + 20);
	...
	\end{minipage}
	 ...
	 More precisely, this branch is a part of the function \texttt{AES\_set\_encrypt\_key()}, whose job is to create a struct containing two fields:
	\begin{itemize}
	...
	The branch is conditioned on the value of \texttt{bits}, which represents the length of the key: it's plausible that the leak appears on the branch with guarding expression \texttt{bits == 128}, since we are analysing AES-128. The output of \textsc{KLEESpectre} also tells us which line loads potentially sensitive data, which in this case is line 671 and 672. Apparently, this doesn't make much sense: those lines are located \textit{outside} the conditional branch, thus speculative execution does not involve them. In order to figure out why \textsc{KLEESpectre} gave such result, I decided to inspect the code to a lower level: when the tool is successfully executed on a file, it creates a directory containing many useful information about the computation. Among these, there is a file with extension \texttt{.ll}, which is an LLVM assembly language format. Moreover, the output of \textsc{KLEESpectre} indicates the assembly instruction corresponding to the C instruction in the source code: with this information, we can track the lines that perform a dangerous loading, according to the tool. These lines are reported on listing \ref{asm1} (note that line numbers follow the same order provided in the output, so they are not sequential).
	
	\lstset{numbers=none}
	\begin{lstlisting}[caption=\texttt{assembly.ll} (for AES-CBC),frame=tlrb, label=asm1]{Name}
...
412:  		%81 = load i8, i8* %arrayidx130, align 1, !dbg !283
	...
	By analysing this code snippet, the result provided by the tool seems more reasonable: the branch \texttt{if(bits == 128)} corresponds to line 311, which is a branch instruction, structured as follows:
	
	\begin{lstlisting}
	br i1 <cond>, label <iftrue>, label <iffalse>
	\end{lstlisting}
		
	For more information about the syntax, see the reference manual \cite{LLVMasm}. In this case, if the branch condition is false, the execution flow jumps at the label \texttt{if.end128}, on line 408, but this label is associated to all the instructions comprehended between line 409 and 469! All the instructions pointed by \texttt{KLEESpectre} are perform a data loading and fall among the instruction under the \texttt{if.end128} label, so the result provided by \textsc{KLEESpectre} looks now more reasonable. 
	\paragraph{}The second vulnerability detected by \textsc{KLEESpectre} is also puzzling: one points at two different files, claiming that the vulnerable branch is in \texttt{cbc128.c}, while the potentially dangerous loading happens in \texttt{aes\_core.c}. One can think of a function in \texttt{cbc128.c} making an external call to a function in \texttt{aes\_core.c}, but when we open the files and look at the corresponding lines, we see that this is not the case, as reported in \ref{cbc128} and \ref{aes-core}.
	...
	\begin{minipage}{.45\textwidth}
		\begin{lstlisting}[caption=\texttt{cbc128.c},frame=tlrb, firstnumber=92, label=cbc128]{Name}
while (len>=16) {
	...
	\end{minipage}\hfill
	\begin{minipage}{.45\textwidth}
		\begin{lstlisting}[caption=\texttt{aes\_core.c},frame=tlrb, firstnumber=797, label=aes-core]{Name}
s0 = GETU32(in) ^ rk[0];
		\end{lstlisting}
	\end{minipage}
	...
	\begin{lstlisting}[caption=\texttt{assembly.ll} (for AES-CBC),frame=tlrb, label=asm2]{Name}
...
861:  		%2 = load i8, i8* %arrayidx, align 1, !dbg !506
	...
	In this case, we can only partially answer our questions: clearly, the \texttt{while} cycle is translated into a jump conditioned on \texttt{cmp1}, which corresponds to an assembly translation of \texttt{len>=16}. If the condition is satisfied, it jumps to the label \texttt{for.body} , otherwise it jumps at \texttt{for.end}. However, none of these blocks contain any reference to the label \texttt{entry}, where instead we find the instruction that, according to \texttt{KLEESpectre}, performs the dangerous loading. Appartently, \texttt{KLEESpectre} treats all branching instructions \texttt{br} equally, regardless of whether these instruction encode an actual branch or a cycle. Despite this, there is still no reason why it detected the dangerous loading in an instruction that has nothing to do with the incriminated branch. 
	...
	\begin{minipage}{.45\textwidth}
		\begin{lstlisting}[caption=\texttt{cbc128.c}, frame=tlrb, firstnumber=105, label=cbc128_2]{Name}
...
(*block)(out, out, key);
	...
	Also in this case, the result is inconsistent: the vulnerable branch is supposed to be the \texttt{for} cycle on line 107, while the dangerous loading seems to be even on the prevous line, which is not reasonable, and on the line immediately after.
	
	The last vulnerability detected by \textsc{KLEESpectre} resides in the file \texttt{memcpy.c}, an alternative implementation of the function \texttt{memcpy(void *dst, const void *src, size\_t len)} which is part of the C standard library: its job is to copy a data of length \texttt{len}, stored at the address \texttt{*srcaddr}, to the address \texttt{*destaddr}. As the implementation is very short, we report in \ref{memcpy} the full code. Once again, the vulnerability seems to involve a cycle, on line 16, instead of a conditional branch, as we would expect.
	
	\begin{minipage}{.9\textwidth}
	\begin{lstlisting}[caption=\texttt{memcpy.c}, frame=tlrb, firstnumber=10, label=memcpy]{Name}
#include <stdlib.h>
...
void *memset(void *dst, int s, size_t count) {
	...
	The vulnerability is very similar to the one in \texttt{memcpy}, as it involves a cycle and the instruction in its body.
	...
	\paragraph{Testing OpenSSL v3} In this paragraph, I repeat the same tests on OpenSSL v3 (more precisely, on version 3.0.0), to see if something has changed after the discovery of Spectre. In order to reproduce these tests, one should run \texttt{test\_KLEESpectre\_v3.sh} and substituting in every testing file the line \texttt{\#include "aes\_locl.h"} with \texttt{\#include "aes\_local.h"}, as the name of the header changed throughout the versions. Figures \ref{fig:result_cbc_v3}, \ref{fig:result_ctr_v3}, and \ref{fig:result_ofb_v3} show the results obtained.
	\begin{figure}[!ht]
		\centering
