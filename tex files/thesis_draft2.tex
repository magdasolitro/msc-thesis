...
for (; n < 16; ++n)
	...
	\begin{figure}[!ht]
		...
		\label{fig:conf-ofb}
	\end{figure}
	As we would expect, the plots look very similar, as they essentially execute the same algorithm with small variations. %To give a more precise evaluation of the degree of similarity between these plot, we compute the Pearson's correlation coefficient (see Appendix \ref{appendix:maths}).
...
\end{minipage}
	...
	As described in Section \ref{sec:spectre-pht}, a Spectre-PHT attack needs a conditional branch to be carried out: this translates into a jump instruction in Assembly (e.g. \texttt{jmp}, \texttt{jbe}, \texttt{jae}).
	
	\begin{comment}
	For example, the gadget shown in listing \ref{spectre-pht-c} translate to the Assembly snipped in \ref{spectre-pht-asm}.
	
	\begin{minipage}{.4\textwidth}
		\begin{lstlisting}[caption={\small Classic Spectre-PHT gadget}, label=spectre-pht-c]
void victim(size_t x) {
	...
	\begin{minipage}{.55\textwidth}
		\begin{lstlisting}[caption={Assembly translation}, label=spectre-pht-asm]
	mov eax, DWORD PTR array1_size
	cmp rcx, rax
	jae SHORT $LN2@victim
	lfence
...
$LN2@victim:
	...
	However, only 3 out of 6 detected gadgets present a jump instruction, namely listings \ref{gadget1}, \ref{gadget4}, and \ref{gadget6}. 
