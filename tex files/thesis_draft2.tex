...
uint8_t foo(uint32_t x) { 
	...
	This appendix begins with a brief introduction to block ciphers and modes of operations, before diving deeper into the details of AES.
	...
	\]
	where $|k|$ represents the key length and $n$ the size of the block. The definition given above means that a ciphertext $c$ is obtained from a key $k$ and a plaintext $p$ as:
	\[
		F(k,p) = c
	\]
	Since the key remains fixed for a certain number of encryptions, before being refreshed, we can parametrise $F$ on $k$, defining $F_k(p) \defeq F(k,p)$.
	...
	There is a clear limitation of block ciphers: they can only encrypt data of fixed size. While the case in which $|p| < n$ can be easily circumvented through padding, if $|p| > n$ we would be forced to discard a part of the message. \textit{Modes of operation} address this problem, providing a way to securely and efficiently encrypt long messages using block ciphers. In simple terms, a mode of operation defines how to concatenate the use of a block cipher in such a way that each cipher encrypts a specific part of the message. In general, if the message $p$ has size $m > n$, the $P$ is split in $\ell = \frac{m}{n}$ blocks, $p_1$, $p_2$, ... , $p_{\ell}$, and the whole ciphertext is obtained as:
	\[
	c = \langle F(k, p_1), F(k, p_2), ..., F(k, p_{\ell})\rangle
	\]
	where the operator $\langle \cdot , ..., \cdot \rangle$ represents a a generic combination. In the rest of the section, we will describe some of the most popular modes of operation.
...
fv 	\end{figure}
	...
	\[
	C = F(k, p_1)\hspace{5px}||\hspace{5px}F(k, p_2)\hspace{5px}||\hspace{5px}... \hspace{5px}||\hspace{5px}F(k, p_{\ell})
	\]
	...
	\[
	\begin{cases}
		c_1 = F_k(IV \oplus p_1)\\
		c_i = F_k(c_{i-1}\oplus p_i) \text{ for all } i>1
	\end{cases}
	\]
	This mode of operation is not deterministic, as the initialisation vector introduces an element of randomness in the encryption. As opposed to the ECB mode, this means that the repeated encryption of the same plaintext results in different ciphertext. This characteristic allows the CBC mode to satisfy the basic standards of security (more precisely, it is CPA-secure\footnote{\url{https://en.wikipedia.org/wiki/Chosen-plaintext\_attack}}).
	Clearly, the choice of the $IV$ plays a fundamental role in the encryption: in order to ensure the indistinguishability of ciphertexts, it's fundamental to choose it uniformly at random.
 	\subsection{Output Feedback (OFB) mode}
 	The Output FeedBack (OFB) mode works similarly to the CBC mode, performing some operations in a different order. Figure \ref{fig:ofb} is a high-level depiction of the OFB mode.
 	\begin{figure}[!ht]
 		\centering
 		...
 		\label{fig:ofb}
 	\end{figure}
 	This mode of operation, like Cipher Block Chaining, also involves a random initialisation vector, whose value must be unpredictable to guarantee the minimum security requirements.
	...
	AES \cite{Katz2007} \cite{AES-FIPS} is a block cipher that was born in 1998 as replacement of the obsolete DES, on request of the U.S. government. At first, its use was limited to military purposes, but nowadays it’s one of the most common block ciphers in use, deeply embedded in various applications. In particular, AES is a subset of the Rijndael ciphers, a family of ciphers with different key and block sizes.
	
	The AES cipher has a 128-bit block length and can use 128-, 192-, or 256-bit keys. In contrast with DES, which uses a Feistel structure, AES is essentially a so-called Substitution-Permutation network.
	AES’s algorithm consist of five main steps:
	\begin{enumerate}
		\item \textsf{KeyExpansion}: a fresh 128-bit key is derived at each round, using the AES key schedule.
		\item \textsf{AddRoundKey}: in every round of AES, a 128-bit sub-key is derived from the master key, and is interpreted as a 4-by-4 array of bytes. The state array is updated by XORing it with this sub-key.
		\item \textsf{SubBytes}: ...
		\item \textsf{ShiftRows}: ...
		\item \textsf{MixColumns}: ...
	\end{enumerate}
	Added to
	In the final round, MixColumns is replaced by AddRoundKey.
	Usually, the number of rounds goes from 10 to 14. 
