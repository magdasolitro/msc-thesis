...
int main(){
	...
	\paragraph{Testing OpenSSL v1} This paragraph shows the results obtained for AES in three different modes of operation, namely Cipher Block Chaining (CBC), Output FeedBack (OFB), and Counter (CTR) mode. Figures \ref{fig:result_cbc_v1} depict the outcome of \textsc{KLEESpectre} on AES-CBC in OpenSSL v1 (more precisely, version 1.0.0), finding 4 Spectre vulnerabilities. 
	...
	\begin{itemize}
		\item the line that starts with \texttt{KLEE:1BR} indicates the branch whose misprediction causes cause data leakage;
	...
	\end{itemize}
	The first thing I noticed is that there are essentially no differences between the two versions of OpenSSL: both contain apparently 4 Spectre vulnerabilities. The next step is going to the lines indicated in the output and see whether the outcome provided by the tool makes sense or not. The first culprit piece of code is the following:
...
rk[5] = GETU32(userKey + 20);
	...
	The leaking branch is in \texttt{aes\_core.c}, that implements a set of functions used in all modes of operations of AES. More precisely, this branch is a part of the function \texttt{AES\_set\_encrypt\_key()}, whose job is to create a struct containing two fields:
	\begin{itemize}
	...
	\begin{lstlisting}
	llvm-dis testing_AES_CBC.bc -o cbc_readable
	\end{lstlisting}
	This command calls the LLVM disassembler, that "takes an LLVM bitcode file and converts it into human-readable LLVM assembly language"\footnote{\url{https://llvm.org/docs/CommandGuide/llvm-dis.html}}. This allows us to ...
	\end{comment}
	I tried to run \textsc{KLEESpectre} also on two more files, \texttt{testing\_AES\_CTR.c} and \texttt{testing\_AES\_OFB.c}, to analyse AES-CTR and AES-OFB respectively: the output is shown in Figures \ref{fig:result_ctr_v1} and \ref{fig:result_ofb_v1}. However, these inspections don't reveal anything new: the vulnerabilities are always found in \texttt{aes\_core.c} (which, I remind, defines functions that are used transversely by all AES modes of operations), \texttt{memcpy.c} and \texttt{memset.c}.
	\begin{figure}[!ht]
		\centering
