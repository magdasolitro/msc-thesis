...
y = x;
	...
	\begin{defn}[Control Flow Graph (CFG)]
		...
		The set $V$ of nodes contains also two special nodes: \textit{START}, that has no predecessors and from which every node is reachable, and \textit{END}, which has no successors and is reachable from every node.
	\end{defn}
	...
	\begin{defn}[Control Dependency Graph (CDG)]
		A \textit{control dependency graph (CDG)} is a directed graph $G=(V,E)$ in which nodes represent the program's variables, while an edge $x\rightarrow y$ expresses the fact that the value of $y$ depends from the value of $x$. 
	\end{defn}
