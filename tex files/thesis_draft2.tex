...
\begin{document}
	...
	Within the harware products market, big companies such as Intel, AMD, Arm strive to produce the faster, smaller, energy-efficient chips to beat the competition and grant themselves the biggest market share. Achieving the best results in every aspect is concretely infeasible: for instance, highly-permorming devices tend to consume a lot of power, while chips must trade small spaces with high memory capacity. Therefore, the main challenge is to reach a decent tradeoff between all the different aspects that condition the performance of a device.

	Speed is a hot topics when it comes to CPU design: essentially, most users want their computer to go \textit{fast}, and to satisfy this request, manifacturers come up every year with all sorts of expedients. One of the most effective dodges to achieve higher performances is called \textit{speculative execution}, which consists in retrieving in advance from the main memory those instructions that have a higher chance to be executed, in order to reduce the time that the processor spends idle, waiting for the instruction and the data it involves to be loaded in the cache. 
	This comes useful in presence of conditional branches: since we cannot statically infer what set of instructions is going to be executed, the CPU uses stastical information about previous executions to \textit{speculate} about how the conditional expression is going to be evaluated, and thus what set of instruction will need to be retrieved.
	
	The introduction of speculative execution successfully improved the CPU performances, but at the same time it introduced some unforeseen vulnerabilities that were discovered only years (?) after these processors were put on the market: Flush+Reload \cite{Yarom2014}, Spectre \cite{Kocher2019}, Meltdown \cite{Lipp2018}, Spoiler \cite{Islam2019} are some of the vulnerabilities that exploit speculative execution to expose potentially secret information. The impact of this discovery was huge, not only because almost all modern processor enforce this kind of execution, but also because it is relatively easy to exploit this vulnerability.
