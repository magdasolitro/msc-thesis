...
uint8_t foo(uint32_t x) { 
	...
	\begin{enumerate}
		...
		\item Comparing the results obtained on AES in its OpenSSL v1.0.0 implementation (last revised in 2015, before the discovering of Spectre) in its OpenSSL v3.0.0 implementation (the latest version), are there any differences? 
	\end{enumerate}
...
int main(){
	...
	I proceed to illustrate the results obtained for AES in three different modes of operation, namely Cipher Block Chaining (CBC), Output FeedBack (OFB), and Counter (CTR) mode. Instead, I excluded the Electronic CodeBook (ECB) mode because, despite being still in use in certain context, it doesn't satisfy the minum security requirements.
