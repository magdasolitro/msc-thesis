...
uint8_t foo(uint32_t x) { 
	...
	\paragraph{Cache modelling} Speculative symbolic execution is further extended with cache modelling: KLEESpectre captures the behaviour of the cache by distinguishing between accesses to \textit{concrete} or \textit{symbolic} memory. While the former represents the access to a memory address that is statically known, the latter stands for the access to an address that can be determined only at run time.
