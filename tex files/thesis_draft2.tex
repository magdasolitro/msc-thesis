...
uint8_t foo(uint32_t x) { 
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[scale=0.35]{imgs/ecb}
		\caption{Electronic Code Book (ECB) mode (source: \cite{Katz2007})}
		\label{fig:ecb}
	\end{figure}
	...
	This mode of operation is not used in real-world applications: since it's deterministic (i.e. by encrypting the same plaintext multiple times, we always obtain the same ciphertext), it can be proved that it doesn't satisfy the minimum standards of security. 
