...
int main(){
	...
	I proceed to illustrate the results obtained for AES in three different modes of operation, namely Cipher Block Chaining (CBC), Output FeedBack (OFB), and Counter (CTR) mode. Instead, I excluded the Electronic CodeBook (ECB) mode because, despite being still in use in certain contexts, it doesn't satisfy the minum security requirements.
	
	Figures \ref{fig:result_cbc_v1} showS the outcome of \textsc{KLEESpectre} on AES-CBC in OpenSSL v1, finding 4 Spectre vulnerabilities. 
...
rk[5] = GETU32(userKey + 20);
	...
	The leaking branch is in \texttt{aes\_core.c}, that implements a set of functions used in all modes of operations of AES. More precisely, this branch is a part of the function \texttt{AES\_set\_encrypt\_key()}, whose job is to create a struct that contains two fields:
	\begin{itemize}
	...
	The branch is conditioned on the value of \texttt{bits}, so it makes sense that the leak appears on the branch with guarding expression \texttt{bits == 128}, since we are analysing AES 128. The output of \textsc{KLEESpectre} also tells us which line loads potentially sensitive data, which in this case is line 671 and 672. Apparently, this doesn't make much sense: those lines are located \textit{outside} the conditional branch, thus speculative execution does not involve them. However, since \textsc{KLEESpectre} analyses LLVM bitcode, it's appropriate to see which instructions are evaluated as dangerous. Since bitcode files are generated with an encoding that is not human readable, I must run the following command from terminal:
	\lstset{
		language=C,
		aboveskip=3mm,
		belowskip=3mm,
	...
	\begin{lstlisting}
	llvm-dis AES_CBC_v1.bc -o readable_bc
	\end{lstlisting}
	...
	I tried to run \textsc{KLEESpectre} also on the \texttt{testing\_AES\_CTR.c} and \texttt{testing\_AES\_OFB.c}, to analyse AES-CTR and AES-OFB respectively: the output is shown in Figures \ref{fig:result_ctr_v1} and \ref{fig:result_ofb_v1}. However, these inspections don't reveal anything new: the vulnerabilities are always found in \texttt{aes\_core.c} (which, I remind, defines functions that are used transversely by all AES modes of operations), \texttt{memcpy.c} and \texttt{memset.c}.
	\begin{figure}[!ht]
		\centering
